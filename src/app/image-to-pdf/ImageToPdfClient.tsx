...
export default function ImageToPdfClient() {
  ...
  const handleFiles = useCallback((newFiles: File[]) => {
    ...
    if (images.length !== newFiles.length) setError("Only image files (JPG, PNG, WebP) are accepted.");
    else setError(null);
  ...
  return (
    <div className="min-h-screen" style={{ background: "linear-gradient(180deg, #fdf8fb 0%, #f8faff 60%, #fff 100%)" }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-10 pb-20">
        <Link href="/" className="inline-flex items-center gap-2 text-sm font-semibold text-gray-500 hover:text-pink-600 transition-colors mb-8 group">
          ...
          <div className="lg:col-span-5 lg:sticky lg:top-24 space-y-8 order-2 lg:order-1">
            <div>
              <div className="flex items-center gap-4 mb-4">
                <div
                  ...
                  style={{
                    ...
                    fontSize: 28,
                    boxShadow: "0 4px 20px rgba(219,39,119,0.12)",
                  }}
                >
                  <FileImage className="w-7 h-7 text-pink-600" />
                </div>
                <div>
                  <h1 className="text-2xl sm:text-3xl font-extrabold text-gray-900 leading-tight">Image to PDF</h1>
              ...
              <p className="text-gray-500 text-sm leading-relaxed mb-0">
                Convert JPG, PNG, and WebP images into a high-quality PDF document instantly. Supports batch conversion.
              </p>
            </div>
            ...
            <div className="flex items-start gap-3 bg-blue-50 border border-blue-100 rounded-2xl p-5">
              ...
              <div>
                <p className="text-sm font-semibold text-blue-800">100% Private</p>
                <p className="text-xs text-blue-600 mt-1 leading-relaxed">
                  Images are processed in memory and never stored permanently. Files are auto-deleted immediately after download.
                </p>
              </div>
            </div>

            <div>
              <p className="section-label mb-4 text-xs font-bold uppercase tracking-wider text-pink-600">Quick Guide</p>
              <ol className="space-y-4">
                {[
                  "Upload one or more images (JPG/PNG).",
                  "Reorder images by dragging (optional).",
                  "Select page size (A4, Letter, or Fit).",
          ...
          <div className="lg:col-span-7 order-1 lg:order-2">
            <div className="card p-5 sm:p-8 shadow-lg border border-gray-100">
              <FileUploadZone
                ...
                sublabel="JPG, PNG, WebP supported — multiple files OK"
                icon={<FileImage className="w-10 h-10 text-pink-400 mb-2" />}
              />
              ...
              {files.length > 0 && (
                <div className="mt-8 space-y-6 animate-fade-in text-left">
                  <div>
                    <label className="block text-sm font-bold text-gray-800 mb-3 uppercase tracking-wider text-[10px]">Page Settings</label>
                    <div className="grid grid-cols-3 gap-3">
                      {(["A4", "Letter", "fit"] as const).map((s) => (
                        <button key={s} onClick={() => setPageSize(s)}
                          className={`py-3 px-2 rounded-xl border-2 text-sm font-bold transition-all ${pageSize === s ? "border-pink-500 bg-pink-50 text-pink-700 shadow-sm" : "border-gray-100 text-gray-400 hover:border-pink-200"}`}>
                          {s === "fit" ? "Fit Image" : s}
                        </button>
                      ))}
                    </div>
              ...
              {files.length > 0 && status === "idle" && (
                <div className="mt-8 flex gap-4">
                  <button onClick={handleConvert} className="btn-primary flex-1 justify-center py-3 text-base" style={{ background: "linear-gradient(135deg, #db2777 0%, #7c3aed 100%)" }}>
                    <FileImage size={18} /> Convert to PDF ({files.length})
                  </button>
                  <button onClick={reset} className="btn-outline px-6">Clear All</button>
                </div>
              )}

              {status === "processing" && <div className="mt-8"><ProgressBar progress={progress} status="processing" label="Creating PDF…" /></div>}

              {status === "done" && downloadUrl && (
                <div className="mt-8 space-y-6 animate-fade-in">
                  <ProgressBar progress={100} status="done" />
                  <div className="flex gap-4">
                    <a href={downloadUrl} download="images.pdf" className="btn-primary flex-1 justify-center py-4 text-base shadow-lg" style={{ background: "linear-gradient(135deg, #db2777 0%, #7c3aed 100%)" }}>⬇️ Download PDF</a>
